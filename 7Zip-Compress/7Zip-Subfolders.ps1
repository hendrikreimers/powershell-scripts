...
$sourceDir  = Resolve-Path -Path $($args[0])
$targetDir  = $sourceDir

$folderList = Get-ChildItem -Directory $sourceDir
...
foreach ( $folderToZip in $folderList ) {
	$source = "$($folderToZip.FullName)"
	$target = "$($targetDir)\$($folderToZip.Name).7z"
	
	# Check if 7z-File already exists
	if ( Test-Path "$($target)" -PathType Leaf ) {
		$newName = "$($folderToZip.FullName)_BAK_$(get-date -Format 'yyyy-MM-dd_HH-mm-ss-tt').7z"
		
		Rename-Item -Path "$($target)" -NewName $newName
		Write-Output ">>> Renamed existing file to `"$newName`" <<<"
	}
	
	Write-Output "Create 7zip of `"$($source)`" to `"$($target)`""
